...
var WebCodeCamJS = function (element) {
    ...
    var display = Q(element),
        ...
        options = {
            ...
            flipHorizontal: true,
            zoom: 0,
            beep: './assets/img/page/beep.mp3',
            decoderWorker: './assets/custom/js/DecoderWorker.js',
            brightness: 0,
    ...
    function gotSources(device) {
        if (device.kind === 'video' || device.kind === 'videoinput') {
            ...
            var textKm = 'camera ' + (videoSelect.length + 1);
            html('<option value="' + (device.id || device.deviceId) + '">' + textKm + '</option>', videoSelect);
        }
    }
