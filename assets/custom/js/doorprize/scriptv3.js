...
const lblAlamat = document.getElementById("lblAlamat");
...
const ubahLabelNama = () => {
  ...
  lblNama.innerHTML = namaPemenang.nama;
    lblAlamat.innerHTML = namaPemenang.alamat;

  if (onProcess === true) {
    setTimeout("ubahLabelNama()", 50);
  } else {
    console.log(idx, namaPemenang.nama);
    // Masukkan nama pemenang ke array
    pemenang.push(namaPemenang.nama);
    // Remove nama dari array yang sudah dipilih agar tidak muncul lagi
    nama.splice(idx, 1);
    // Tampilkan pemenang
    tampilkanPemenang(namaPemenang.nama);
  }
};
